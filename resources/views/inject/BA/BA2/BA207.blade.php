...
//			kegSubData[kegIndex].data.body_num = ( parseFloat( row['data']['body_num'] / row['data']['body_rate']) + parseFloat(num)).toFixed(2);
    ...
    window.injects.injectOnAdd.push((that, pageData) => {
		...
		formVue.$set(formVue.dataset.override.fields, 'print_quot' ,{ field_readonly : true });
		   ...
		   if( fromField.field_code == "depot_code" ){
			    ...
			    sendAPIRequest(getURL(`api/inject/changeProductCode`),"post",productData).then(result => {
				   if( result['product_code'] !== "undefined" ){
					...
					osubtotal =osubtotal = countSubtotal2(row,subDataArray,parentDataset,"body_subtotal","body_num","o_body_price","discount","osubtotal");
					countOriginal(osubtotal,parentDataset,"osubtotal","otax","ototal",taxrate,bodyId1,"body_subtotal");
					countStandard(osubtotal,parentDataset,rate,"ssubtotal","stax","stotal","otax","ototal");
...
//				bodyNumRecord[index] = rowRum;
    ...
    window.injects.injectOnRowDelete.push((that, pageData, parentDataset, formID, rowIndex) => {
		...
		let rate = parentDataset.data.rate;
		let taxrate = parentDataset.data.taxrate;
		if(that.dataset.subData[2199][rowIndex].data.body_subtotal != null && that.dataset.subData[2199][rowIndex].data.body_subtotal !=  ".00"){
		osubtotal = parseFloat(that.dataset.data.osubtotal- parseFloat(that.dataset.subData[2199][rowIndex].data.body_subtotal)).toFixed(2);
		osubtotal2 = parseFloat(that.dataset.data.ototal- parseFloat(that.dataset.subData[2199][rowIndex].data.body_subtotal)).toFixed(2);
...
//								console.log(subData[index]['data']['source_keg']['subData'][kegSubFormId][0].data.body_num);
    ...
    window.injects.injectOnHeadClick.push((that, pageData, field, dataset) => {
		const filedCode = field.field_code;
		 if( pageData.page.page_id == mainPageId ){
			if( filedCode == 'print_quot'){
					let docu_number = {
						docu_number:that.dataset.data.docu_number,
					}
					console.log(docu_number);
					fullscreenDimmer.loading()
					sendAPIRequest(getURL(`api/inject/printOrder`),"post",docu_number).then( async result => {
						if(result.status){
            ...
            else if( pageData.page.page_id == mainPageId &&　filedCode == "trans_orders" ){
				...
				sendAPIRequest(getURL(`api/inject/transToOrder`),"post",orderData).then(result => {
			   //console.log(result);
			   alert(result['text']);
...
//					editStatus = false;
    ...
    window.injects.injectOnBodyChange.push((that, pageData, parentDataset, row, field, formID, subDataArray) => {
		...
		if( pageData.page.page_id == mainPageId ){
			...
			if(filedCode == "body_num" || filedCode == "body_price" || filedCode == "o_body_price" || filedCode == "discount" ){

				osubtotal = countSubtotal2(row,subDataArray,parentDataset,"body_subtotal","body_num","body_price","osubtotal","discount");
				//原幣
				let taxrate = parentDataset.data.taxrate;
				countOriginal(osubtotal,parentDataset,"osubtotal","otax","ototal",taxrate,bodyId1,"body_subtotal");
				//本位幣
