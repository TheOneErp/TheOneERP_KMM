...
<script>
			...
			if( filedCode == 'charge_off' ){
                    ...
                    for(let i=0;i<=indexlen-1;i++){
                        if(that.dataset.subData[bodyId1][i].data.choose=="1"){
                            if(that.dataset.subData[bodyId1][i].data.source_code!=null){
                                ...
                                recdamtarray.push(that.dataset.subData[bodyId1][i].data.recd_amt);
                            }
                        }
                    }
                    data1={
                    ...
                    sendAPIRequest(getURL(url),"post",data).then(async result => {
                        ...
                        if(indexlen>0){
                            for(j=0;j<indexlen;j++){
                                ...
                                that.dataset.subData[bodyId1][j].data.amt_receivable=result[j]["ototal"];
                                that.dataset.subData[bodyId1][j].data.amt_outstanding=result[j]["receivable"];
                                that.dataset.subData[bodyId1][j].data.recd_amt=result[j]["recd_amt"];
                ...
                for(let i=0;i<=indexlen-1;i++){
                    if(that.dataset.subData[bodyId1][i].data.source_code!=null){
                        ...
                        sum=parseFloat(Number(sum)+Number(formVue.dataset.subData[bodyId1][i].data.recd_amt)).toFixed(2);
                        sum1=parseFloat(Number(sum1)+Number(formVue.dataset.subData[bodyId1][i].data.amt_discount)).toFixed(2);
                    }
                }
    ...
    window.injects.injectOnBodyChange.push((that, pageData, parentDataset, row, field, formID, subDataArray) => {
		...
		if( pageData.page.page_id == mainPageId ){
			if( filedCode == 'choose' ){
                ...
                if(formVue.dataset.subData[bodyId1][index].data.choose=="1"){
                    if(formVue.dataset.subData[bodyId1][index].data.source_code!=null){
                        ...
                        formVue.dataset.data.charge_total=parseFloat(Number(formVue.dataset.data.charge_total)+Number(formVue.dataset.subData[bodyId1][index].data.recd_amt)).toFixed(2);
                        formVue.dataset.data.discount_total=parseFloat(Number(formVue.dataset.data.discount_total)+Number(formVue.dataset.subData[bodyId1][index].data.amt_discount)).toFixed(2);
                    }
            ...
            }else if(filedCode == 'amt_discount'){
                let indexlen=that.dataset.subData[bodyId1].length;
                let sum = 0;
                for(let i=0;i<=indexlen-1;i++){
                    if(that.dataset.subData[bodyId1][i].data.source_code!=null){
                        if(that.dataset.subData[bodyId1][i].data.choose=="1"){
                            sum=parseFloat(Number(sum)+Number(formVue.dataset.subData[bodyId1][i].data.amt_discount)).toFixed(2);
                        }
                    }
                }
                formVue.dataset.data.discount_total=sum;
                let currentDataset = parentDataset.subData[bodyId1]
                let schema = that.config.schema[bodyId1]
                let firstRowEmptyFlag = false
                currentDataset.forEach((row, rowIndex) => {
