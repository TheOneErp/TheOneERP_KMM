...
//			kegSubData[kegIndex].data.body_num = ( parseFloat( row['data']['body_num'] / row['data']['body_rate']) + parseFloat(num)).toFixed(2);
    ...
    window.injects.injectOnAdd.push((that, pageData) => {
        formVue.$set(formVue.dataset.override.fields, 'trans_orders' ,{ field_readonly : true });
        ...
        if(localStorage.hasOwnProperty('shipdata') == true){
            ...
            for( let key in orderdata) {
                if(key == 'client_code'){
                    ...
                    formVue.dataset.data.tax = orderdata[key];
                }else if(key == 'taxrate'){
                    formVue.dataset.data.taxrate = orderdata[key];
            ...
            osubtotal = osubtotal = counSubtotaldiscount(row,subDataArray,parentDataset,"body_subtotal","body_num","body_price","discount","osubtotal");
...
//				bodyNumRecord[index] = rowRum;
				...
				if( theSame == 0 ){
					...
					for( let element of that.parentVue().dataset.subData[bodyId1] ){
						if( element.selected ){
							...
							element.data.body_subtotal = parseFloat(element.data.body_num*element.data.body_price*element.data.discount/100).toFixed(2);
    ...
    window.injects.injectOnRowDelete.push((that, pageData, parentDataset, formID, rowIndex) => {
		...
		let rate = parentDataset.data.rate;
		let taxrate = parentDataset.data.taxrate;
		if(that.dataset.subData[bodyId1][rowIndex].data.body_subtotal != null && that.dataset.subData[bodyId1][rowIndex].data.body_subtotal !=  ".00"){
		osubtotal = parseFloat(that.dataset.data.osubtotal- parseFloat(that.dataset.subData[bodyId1][rowIndex].data.body_subtotal)).toFixed(2);
		osubtotal2 = parseFloat(that.dataset.data.ototal- parseFloat(that.dataset.subData[bodyId1][rowIndex].data.body_subtotal)).toFixed(2);
    ...
    window.injects.injectOnHeadChange.push((that, pageData, field, dataset) => {
		...
		if( pageData.page.page_id == mainPageId ){
			...
			let rate = dataset.data.rate;
			let osubtotal = dataset.data.osubtotal;
			if( dataset.data.tax == "税內含" ){
				console.log(dataset.data.tax);
			}

			else if( filedCode == "taxrate" ){
					...
					sendAPIRequest(getURL(`api/inject/getBucketProduct`),"post",bucketData).then(result => {
						...
						if(result['status'] == 1){
							...
							for (let element of subData) {
								...
								if( element.data.product_code == kegProductCode && (element.data.body_depot_code == kegDepoCode || !element.data.body_depot_code) ){
									...
									if( kegForm == 0 || kegForm == 2 ){
										...
										that.$set(element.override.fields, 'body_depot_code' ,{ field_readonly : true });
										osubtotal = counSubtotal(element,subData,dataset,"body_subtotal","body_num","body_price","osubtotal","discount");
										break;
									}else{
										break;
									}
							...
							if( productExist == 0 ){
								...
								subData[index]['data']['body_subtotal'] = parseFloat(result['data']['body_price'] * num  * (subData[index]['data']['discount']/100)).toFixed(2);
								...
								osubtotal = counSubtotal(subData[index],subData,dataset,"body_subtotal","body_num","body_price","osubtotal","discount");
//								console.log(subData[index]['data']['source_keg']['subData'][kegSubFormId][0].data.body_num);
    ...
    window.injects.injectOnHeadClick.push((that, pageData, field, dataset) => {
		const filedCode = field.field_code;
		 if( pageData.page.page_id == mainPageId ){
			if( pageData.page.page_id == mainPageId &&　filedCode == "trans_orders" ){
                let readArr = ['advanceday','client_code','client_name','docu_number','currency','osubtotal','otax','ototal','rate','remarks','ssubtotal','stax','stotal','tax','taxrate','undertaker','undertakerday','undertakername','transport_code'];
				if( editStatus && machiningCodeEmpty ){
		   			let orderData = {
						docu_number : that.dataset.data.docu_number,
					action : formVue.dataset.status
				};
				sendAPIRequest(getURL(`api/inject/transToOrder1`),"post",orderData).then(result => {
			   console.log(result);
			   alert(result['text']);
...
//					editStatus = false;
			...
			if( !editStatus ){
			   alert("此報價單已被修改，請先儲存再轉採購單");
			}else if( !machiningCodeEmpty ){
				alert("此客戶訂單表身皆已轉客戶訂單");
			}
    ...
    window.injects.injectOnBodyChange.push((that, pageData, parentDataset, row, field, formID, subDataArray) => {
		...
		if( pageData.page.page_id == mainPageId ){
			...
			//console.log(parentDataset.data['tax']);
			if(filedCode == "body_num" || filedCode == "body_price" || filedCode == "discount" ){

				let index = formVue.dataset.subData[bodyId1].findIndex(x => x.selected);
				afterdiscount = row.data.body_price*row.data.body_num*row.data.discount/100;
				formVue.dataset.subData[bodyId1][index].data.body_subtotal=parseFloat(afterdiscount).toFixed(2);
				//console.log(parentDataset.data.tax);
				osubtotal = counSubtotaldiscount(row,subDataArray,parentDataset,"body_subtotal","body_num","body_price","discount","osubtotal");
				//console.log(formVue.dataset.subData[bodyId1][index].data.body_subtotal);
				//原幣
				let taxrate = parentDataset.data.taxrate;
				countOriginal(osubtotal,parentDataset,"osubtotal","otax","ototal",taxrate,bodyId1,"body_subtotal");
				//本位幣
