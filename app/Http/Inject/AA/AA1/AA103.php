...
class AA103 extends InjectBase
{
    static public function bfDelete(&$data,$verify = false){
        ...
        foreach($results as $key => $val){
            if($val->t != 'AA103' && $val->t != 'BA204' && $val->t != 'BA205' && $val->t != 'BA206' && $val->t != 'CA204' && $val->t != 'CA205' && $val->t != 'DA203' && $val->t != 'EA204' && $val->t != 'BA301' && $val->t != 'CA301' && $val->t != 'DA301'&& $val->t != 'BA210'  && $val->t != 'CA210'){
                if($val->t == 'AA202'){
                    ...
                    $sfield = $val->name == 'AA202_31' ? 'body_unit_code' : 'unit_code';
        ...
        //加工單、完工單子件如果有被引用也不能刪
        $da201 = DB::table('DA201_46')
                ->select('*')
                ->where('component_unit', $code)
                ->get();
        if(count($da201) > 0){
            $translations = TranslationUtil::getTranslationByCode('DA201');
            $pagecode = $pagecode."「".$translations."」";
        }
        $da202 = DB::table('DA202_58')
                ->select('*')
                ->where('component_unit', $code)
                ->get();
        if(count($da202) > 0){
