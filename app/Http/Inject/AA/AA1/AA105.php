...
namespace App\Http\Inject\AA\AA1;
...
class AA105 extends InjectBase
{
...
//        dd($results);
        ...
        foreach($results as $key => $val){
            if($val->t != 'AA103' && $val->t != 'AA104' && $val->t != 'AA105' && $val->t != 'BA204' && $val->t != 'BA205' && $val->t != 'BA206' && $val->t != 'CA204' && $val->t != 'CA205' && $val->t != 'DA203' && $val->t != 'EA204' && $val->t != 'BA301' && $val->t != 'CA301' && $val->t != 'DA301'){
                $indata = DB::table($val->name)
                        ->select('*')
                        ->where($val->colname, $code)
                        ->get();
                if(count($indata) > 0){
    ...
    static public function beforeDelete(&$data, &$pageData)
    {
		...
		if ( !VerifyUtil::pageVerifyConfirmation($pageId) ) {
            ...
            if($res['pagecode'] != ''){
                response()->json(['status' => false , 'message' => '單位代碼：'.$res['code'].'  已被引用於：'.$res['pagecode'].'，故無法刪除'])->send();
                die();
            }
        }
    }
    ...
    static public function afterLastestReturnVerify(&$data, &$result){
        ...
        if($tmpArr['pagecode'] != ''){           
            $result["messages"] = ['單位代碼：'.$tmpArr['code'].'  已被引用於：'.$tmpArr['pagecode'].'，故無法退回'];
	...
	static public function afterLastestInitVerify(&$data, &$result){
		$tmpArr = self::bfDelete($data,true);
        if($tmpArr['pagecode'] != ''){           
            $result["messages"] = ['單位代碼：'.$tmpArr['code'].'  已被引用於：'.$tmpArr['pagecode'].'，故無法重置'];
            $result["success"] = false;
        }
	}
}
