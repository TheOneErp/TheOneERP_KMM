...
class CA303{
    ...
    public function __construct($filters){
        ...
        $CA202data = DB::table('CA202_54 as a')
           ...
           END AS decimal(10,2)) as body_subtotal,a.remarks,b.body_remarks,b.pay_status,b.id as idid,c.vendor_cat_name"))
                ...
                ->leftJoin('CA102_27 as c', 'a.vendor_code','=','c.vendor_code');
// dd($CA202data->get());
        ...
        WHEN a.tax = '稅外加'
           THEN b.body_subtotal * ( 1 +  CAST(('0' + a.taxrate) AS float) )
        ELSE b.body_subtotal
   END AS decimal(10,2))  as body_subtotal2 ,a.remarks,b.source_receive_code,b.id as idid,c.vendor_cat_name"))
		...
		if( !empty($filters['s_vendor_code']) ){
			$CA202data = $CA202data->where('a.vendor_code', '>=', $filters['s_vendor_code']);
            $CA202_receivecode = $CA202_receivecode->where('a.vendor_code', '>=', $filters['s_vendor_code']);
		}

		if( !empty($filters['e_vendor_code']) ){
			$CA202data = $CA202data->where('a.vendor_code', '<=', $filters['e_vendor_code']);
            $CA202_receivecode = $CA202_receivecode->where('a.vendor_code', '<=', $filters['e_vendor_code']);
		}
        ...
        foreach ($CA202data as $key => $row) {
            ...
            if($row->receive_code){
                $row->code=$row->receive_code;
            }else{
                $row->code=$row->abort_code;
            }
        }
        ...
        $code = array_column($CA202data, 'code');
        $idid = array_column($CA202data, 'idid');
       ...
       if($CA202data != null){
        array_multisort($datas, SORT_ASC, $code, SORT_ASC,$idid, SORT_ASC, $CA202data);
        }
