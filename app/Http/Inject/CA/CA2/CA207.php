...
class CA207 extends InjectBase
{
    ...
    {
        $checkorder=DB::table('CA201_42')->where('source_code','=',$data['data']["docu_number"])->get();
        //dd($checkorder);
        if( $checkorder->count() !=  0 ){
            response()->json(['status' => false , 'message' => '此報價單已轉為採購單，故無法刪除'])->send();
            die();
        }
    }
