...
class RE101 extends InjectBase
{
    static public function bfDelete(&$data,$verify = false){
        ...
        $code = $data['data']['cat_id'];
        // dd($code,$warm);
        $delmessage = CommonController::AreUsedfordelete($code,'RE101','cat_id');
        // dd($delmessage);
        // if($verify){
        //   $warm[] = $delmessage;
        // }else{
    ...
    static public function beforeDelete(&$data, &$pageData)
    {
        ...
        if ( !VerifyUtil::pageVerifyConfirmation($pageId) ) {
            ...
            //            $translations = TranslationUtil::getTranslationByCode('CA202.error.deletefailed');
                response()->json(['status' => false , 'message' => '類別代碼：'.$code.'  已被引用於：'.$warm.'，故無法刪除'])->send();
                DB::rollback();
                die();
            }else{
                DB::commit();
    ...
    static public function afterLastestReturnVerify(&$data, &$result){
        ...
        $code = $data['data']['cat_id'];
        if( !empty($tmpArr) ){
            $result["messages"] = ['類別代碼'.$code.'  已被引用於：'.$tmpArr.'，故無法退回'];
