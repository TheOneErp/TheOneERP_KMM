...
class BA211 extends InjectBase
{
	...
	static public function bfSave(&$data,$verify = false){
		...
		if($data['status'] == 'update'){
            ...
            $oldBA202data = DB::table('BA211_6263')->where('id', $data['data']['id'])->first();
			...
			$oldbodydata = DB::table('BA211_6264')->where('parent_id', $data['data']['id'])->get();
			$totalnum = 0;
			$totalquantity = 0;
		...
		if( array_key_exists('subData',$data) ){
			...
			foreach($data['subData'][$subDataFormId] as $key=>$value){
				...
				$final_pmt = $data['data']['final_pmt'];
				if($final_pmt == 0 || $final_pmt == '0'|| $final_pmt == '.00'){
					$value['data']['payment_staus'] = "已收款";
				}
...
if($value['data']['combi_code']==""||$value['data']['combi_code']==null){
	...
	static public function atSave(&$data,$verify = false)
	{
		...
		if (VerifyUtil::pageVerifyConfirmation(6264)) {
		...
		if( array_key_exists('subData',$data) ){
			foreach($data['subData'][$subDataFormId] as $datakey => $dataval){
				...
				$final_pmt = $data['data']['final_pmt'];
				if($final_pmt == 0 || $final_pmt == '0'|| $final_pmt == '.00'){
					$dataval['data']['payment_staus'] = "已收款";
				}
				/* if( $verify ){
			...
			$depotPageId = '58';
			$kegPageId = '1070';
			$ship_id = DB::table('BA211_6263')->where('ship_code', $data['data']['ship_code'])->pluck('id')->first();
			//轉出庫存調整單，桶號調整單
