...
class BA212 extends InjectBase
{
            ...
            foreach($data['subData'][$subDataFormId] as $datakey => $value){
                if($value['data']['choose'] == '1'){
                    ...
                    if (strpos($row, 'BA203') !== false) {
                        ...
                        $HeadData = DB::table("BA211_6263 as a")
                            ->where('ship_code', '=', $row)
                            ->first();
                        $SubData = DB::table("BA211_6264 as b")
                            ->select(DB::raw("b.id"))
                            ->leftJoin('BA211_6263 as a', 'a.id', '=', 'b.parent_id')
                            ->where('ship_code', '=', $row)
                            ->get();
                        $checkData = DB::table("BA212_6266 as a")
                            ->where('source_code', '=', $row)
                            ->where('choose', '=', '1')
                            ->count();
                        $BA212data = DB::table("BA212_6266 as b")
                            ->leftJoin('BA212_6265 as a', 'a.id', '=', 'b.parent_id')
                            ->where('source_code', '=', $row)
                            ->where('choose', '=', '1')
                            ->orderby('a.id','desc')
                            ->first();
                        ...
                        $amt_recd = $HeadData->amt_recd - $value['data']['recd_amt'] ;
                        $final_pmt = $HeadData->ototal - ($HeadData->amt_recd - $value['data']['recd_amt'] );
                        ...
                        DB::table("BA211_6263")
                            ...
                            foreach($SubData as $key1=>$row1 ){
                                DB::table("BA211_6264 as a")
                                    ->leftJoin('BA211_6263 as b', 'b.id', '=', 'a.parent_id')
                                    ->where('a.id', '=', $row1->id)
                                    ->update([
                                        'payment_status' => "未收款",
                                    ]);
