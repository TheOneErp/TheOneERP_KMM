...
class BA101 extends InjectBase
{
    static public function bfDelete(&$data,$verify = false){
        ...
        foreach($results as $key => $val){
            if($val->t != 'BA101' && $val->t != 'BA204' && $val->t != 'BA205' && $val->t != 'BA206' && $val->t != 'CA204' && $val->t != 'CA205' && $val->t != 'DA203' && $val->t != 'BA301' && $val->t != 'CA301' && $val->t != 'DA301'){
                $indata = DB::table($val->name)
                        ->select('*')
                        ->where('client_cat', $code)
                        ->get();
                if(count($indata) > 0){
    ...
    static public function beforeDelete(&$data, &$pageData)
    {
		...
		if ( !VerifyUtil::pageVerifyConfirmation($pageId) ) {
			// DB::beginTransaction();
            $res = $warm = self::bfDelete($data);
            if($res['pagecode'] != ''){
                response()->json(['status' => false , 'message' => '客戶類別代碼：'.$res['code'].'  已被引用於：'.$res['pagecode'].'，故無法刪除'])->send();
                die();
            }
        }
        //dd($data);
    ...
    static public function afterLastestReturnVerify(&$data, &$result){
        ...
        if($tmpArr['pagecode'] != ''){           
            $result["messages"] = ['客戶類別代碼：'.$tmpArr['code'].'  已被引用於：'.$tmpArr['pagecode'].'，故無法退回'];
	...
	static public function afterLastestInitVerify(&$data, &$result){
		$tmpArr = self::bfDelete($data,true);
        if($tmpArr['pagecode'] != ''){           
            $result["messages"] = ['客戶類別代碼：'.$tmpArr['code'].'  已被引用於：'.$tmpArr['pagecode'].'，故無法重置'];
            $result["success"] = false;
        }
	}   
}
